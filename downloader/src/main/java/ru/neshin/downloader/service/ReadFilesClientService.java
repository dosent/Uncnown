...
public class ReadFilesClientService extends WebServiceGatewaySupport {
    ...
    private void downloadFile() {
        ...
        if (wasSaveLocalFile == null) {
            LOGGER.info("Надо загрузить послению максимальную версию");
            ...
            LOGGER.info("Загружена максимальная версия, ничего делать не надо");
        } else {
            downloadFile(savedLocalFile, FILE_PATH, false);
            LOGGER.info("нужно загружать обновления");
        }
    }
...
//            FileUtils.copyURLToFile(url, file);
            ...
            client.prepareGet(String.valueOf(url))
                    ...
                    .execute(new AsyncCompletionHandler<FileOutputStream>() {
                        ...
                        public State onBodyPartReceived(HttpResponseBodyPart bodyPart)
                            ...
                            stream.flush();
                            isReadyToWork = false;
                            LOGGER.debug("Пишу " + (stream.getChannel().size() / 1024 / 1024) + " Mb");
                        ...
                        public FileOutputStream onCompleted(Response response)
                            ...
                            unRarService.extract(file.getAbsolutePath(), file.getAbsolutePath().replace(".rar",""));
                            isReadyToWork = true;
    ...
    private void readFromFaip() {
        LOGGER.info("Read web service start");
        try {
            ...
            LOGGER.error("Error read web service:");
            e.printStackTrace();
        }
        LOGGER.info("Read web service stop");
    }
